...
| Brief, non-interactive supplemental help                  | `.tooltip` + trigger `aria-describedby` + `.tip[role="tooltip"]`                        |
...
| Editorial/docs reading layout with optional side rails     | `.layout-holy-grail` with direct `.rail-start` / `<main>` or `<article>` / `.rail-end` children                  |
...
| Three equal responsive columns (cards, comparisons)        | `.layout-three-col`                                                                                              |
...
`.rail-start` and `.rail-end` are optional direct-child contracts of `.layout-holy-grail`, not free-floating utilities. Keep semantic DOM order as start rail → centered `<main>` or `<article>` → end rail. At 1024px and below the end marginalia is hidden; at 768px and below a plain start rail is hidden. To retain the start rail on mobile, use a direct `.rail-start.drawer[popover]` child and target it from a `.drawer-toggle`; the native popover pattern needs no JavaScript.
...
| Accessible tooltip                          | `.tooltip` + non-interactive `.tip[role="tooltip"]` description        |
...
| Bottom-pinned bar respecting home indicator | `padding-block-end: calc(var(--safe-bottom) + var(--pad-s))` |
...
| `.tip[aria-label]` pseudo content                           | `.tip[data-tooltip]` on an explicitly named icon-only control |
